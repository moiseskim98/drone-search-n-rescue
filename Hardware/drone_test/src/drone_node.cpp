...
#define Point_P_x 0.006
#define Point_D_x 0 //0.001
#define Point_P_y 0.004
...
geometry_msgs::Point current_fan;
...
void fan_cb(const geometry_msgs::Point::ConstPtr &msg)
{
	current_fan = *msg;
}
/***************************************call backs end****************************************/
...
int main(int argc, char **argv)
{
	...
	ros::Subscriber fan_sub = nh.subscribe<geometry_msgs::Point>("/camera_front/pose", 10, fan_cb);
	...
	for (int i = 500; i >= 0; i--) //10s fix
	{
		...
		if (confidence_circle >= 0.7)
		{
			...
			err.x = (current_point.x + 30); //-30,0
			err.y = current_point.y;
			if (0) //(fabs(err.x)<=10&&fabs(err.y)<=10)
			{
				set_break();
			}
			else
			{
				...
				out.x *= confidence_circle;
				out.y *= confidence_circle;
				out.x = bound<double>(out.x, 0.4);
				out.y = bound<double>(out.y, 0.4);
				//     set_speed_body(out.x,out.y,0,0);//flu
				set_pose_body(out.x, out.y, 1.0 - current_pose.pose.position.z, 0);
				ROS_INFO("fixing...");
			}
		}
	...
	for (int i = 500; i >= 0; i--) //5s line_follow
	{
		...
		{
			double speed_x = 0.10;
			double speed_y = current_line.y / 300.0;
			...
			//    set_pose_body(speed_x,speed_y,1.5-current_pose.pose.position.z,yaw_rate);
			ROS_INFO("following1...");
		}
		else
		{
			set_speed_body(0.1, 0, (1.0 - current_pose.pose.position.z) / 2.0, 0); //flu
		}
		ros::Duration(0.02).sleep();
	}
	...
	for (int i = 500; i >= 0; i--) //10s fix
	{
		...
		if (confidence_circle >= 0.7)
		{
			geometry_msgs::Point err;
			geometry_msgs::Point out;
			err.x = (current_point.x + 48); //-46,15
			err.y = (current_point.y - 12);
			if (0) //(fabs(err.x)<=10&&fabs(err.y)<=10)
			{
				set_break();
			}
	...
	set_pose_body(-0.15, 0,  1.3-current_pose.pose.position.z, 0); //back by 15 cm
	...
	for (int i = 1000; i >= 0; i--) //20s(max) fix_fan
	{
		ros::spinOnce();
		geometry_msgs::Point err;
		geometry_msgs::Point out;
		err.x = current_fan.x; //-46,15
		err.y = current_fan.y;
		if (fabs(err.x)<=10)
		{
			set_break();
		...
		else
		{
			...
			out.y = bound<double>(out.y, 0.4);
			//set_speed_body(out.x,out.y,0,0);//flu
			set_pose_body(0, out.x, 1.3-current_pose.pose.position.z, 0);
			ROS_INFO("fixing fan...");
		}
		ros::Duration(0.02).sleep();
	}

	set_pose_body(0.15, 0, 0, 0); //forward by 15 cm
	for (int i = 0; i < 300; i++) //4s
	{
		ros::spinOnce();
		ros::Duration(0.01).sleep();
	}
	set_pose_body(0, 0, 0.25, 0); //up by 25cm
	for (int i = 0; i < 400; i++) //4s
	{
		ros::spinOnce();
		ros::Duration(0.01).sleep();
	}
	camera_front_capture();
	set_pose_body(0, 0, -0.25, 0); //back
	for (int i = 0; i < 400; i++) //4s
	{
		ros::spinOnce();
		ros::Duration(0.01).sleep();
	}
	set_pose_body(0, 0.2165, -0.125, 0); //left blade
	for (int i = 0; i < 400; i++) //4s
	{
		ros::spinOnce();
		ros::Duration(0.01).sleep();
	...
	camera_front_capture();
	set_pose_body(0, -0.2165, 0.125, 0); //back
	for (int i = 0; i < 400; i++) //4s
	{
		ros::spinOnce();
		ros::Duration(0.01).sleep();
	}
	set_pose_body(0, -0.2165, -0.125, 0); //right blade
	for (int i = 0; i < 400; i++) //4s
	{
		ros::spinOnce();
		ros::Duration(0.01).sleep();
